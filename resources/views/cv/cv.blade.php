...
<body class="flex flex-col justify-center content-center">
    <!-- outer container -->
        ...
        <header>
            ...
            <div class="flex justify-between items-center">
                ...
                <div class="grid justify-items-end">
                    ...
                    <p class="text-xl mt-5">{{ $utilisateur->titre }}</p>
                </div>
            </div>
        </header>
        <!-- detailed info -->
        <main class="flex gap-x-10 mt-10">
            <div class="w-2/6">
                ...
                <ul class="mt-2 mb-10">
                    <li class="px-2 mt-1"><strong class="mr-1">Phone </strong>
                        <a href="tel:+821023456789" class="block">{{ $utilisateur->telephone }}</a>
                    </li>
            ...
            <div class="w-4/6">
                <section>
                    ...
                    <p class="mt-4 text-xs">{{ $utilisateur->a_propos }}</p>
                ...
                <section>
                    ...
                    <!-- work experiences -->
                    <h2 class="text-2xl mt-6 pb-1 border-b font-semibold">Professionell Experiences</h2>
                    <ul class="mt-2">
                        @foreach ($experiences as $experience)
                        <li class="pt-2">
                            <p class="flex justify-between text-sm"><strong class="text-base">{{ $experience->company }}</strong>{{ $experience->start_year }}-{{ $experience->end_year }}</p>
                            <p class="flex justify-between text-base">{{ $utilisateur->titre }}<small>  {{ $experience->position }}</small></p>
                ...
                <section>
                    ...
                    @foreach ($cursuses as $cursus)
                        <li class="pt-2">
                            <p class="flex justify-between text-sm"><strong class="text-base">{{ $cursus->institution }}</strong>{{ $cursus->start_year }}-{{ $cursus->end_year }}</p>
                            <p class="flex justify-between text-sm">{{ $cursus->degree }}<small>GPA 4.0</small></p>
                        </li>
                        @endforeach
...
</body> <a href="{{ route('generate.pdf') }}" class="block w-full max-w-xs mx-auto bg-green-500 hover:bg-green-600 text-white font-semibold text-center rounded p-3">Download PDF</a>
